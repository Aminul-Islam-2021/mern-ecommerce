...
import { ChevronDownIcon, ChevronUpIcon, SearchIcon, StarIcon } from '@heroicons/react/solid';
...
const ProductsPage = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-gray-100 min-h-screen">
      {/* Left Sidebar */}
      <div className="w-full lg:w-1/4 p-4 bg-white shadow-lg rounded-lg space-y-6">
          ...
          {openSections.price && (
            <div className="mt-4">
              <Range
                ...
                onChange={(values) =>
                  setFilters({ ...filters, price: { ...filters.price, currentRange: values } })
                }
                ...
                renderThumb={({ props }) => (
                  <div {...props} className="w-5 h-5 bg-blue-600 rounded-full shadow"></div>
                )}
              />
              ...
              {filters.colors.map((color) => (
                <li key={color}>
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border-2 ${color} ${filters.selectedColor === color ? 'border-gray-800' : 'border-gray-300'}`}
                  />
                </li>
              ))}
            </ul>
              ...
              {[5, 4, 3, 2, 1].map((star) => (
                <button
                  ...
                  className={`flex items-center space-x-1 p-1 ${
                    filters.rating === star ? 'text-yellow-500' : 'text-gray-400'
                  }`}
                >
                  {Array(star)
                    .fill(0)
      ...
      <div className="w-full lg:w-3/4 p-4 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {products
            ...
            .filter(
              (product) =>
                filters.activeFilters.includes(`category: ${product.category}`) ||
                filters.activeFilters.length === 0
            )
            .map((product) => (
        ...
        <div className="mt-6 flex justify-center space-x-2">
          <button className="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg">Prev</button>
          {/* Page numbers can be dynamically created */}
          {[1, 2, 3].map((page) => (
            <button
              key={page}
